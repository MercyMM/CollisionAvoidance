...
IplImage* drawFloatGraph(const float *arraySrc, int nArrayLength, IplImage *imageDst, float minV, float maxV, int width, int height, char *graphLabel, bool showScale)
{
	...
	if (showScale) {
		...
		CvScalar clr = GREY;
		char text[16];
//		cvPutText(imageGraph, cv::format("%.1f", maxV), cvPoint(1, b+4), &font, clr);
		putText(imageCpp, cv::format("%.1f", maxV),   Point(1, b+4), FONT_HERSHEY_PLAIN, 0.5, GREY, 1);
		// Write the scale of the x axis
//		sprintf_s(text, sizeof(text)-1, "%d", (nArrayLength-1) );
//		cvPutText(imageGraph, text, cvPoint(w-b+4-5*strlen(text), (h/2)+10), &font, clr);
		putText(imageCpp, cv::format("%d", (nArrayLength-1)),   Point(w-b+4-5*strlen(text), (h/2)+10), FONT_HERSHEY_PLAIN, 0.5, GREY, 1);
	}
...
// Draw the graph of an array of floats into imageDst or a new image, between minV & maxV if given.
// Remember to free the newly created image if imageDst is not given.
Mat drawFloatGraph2(std::deque <double> arraySrc, int nArrayLength, Mat imageDst, float minV, float maxV, int width, int height, char *graphLabel, bool showScale)
{
	if (nArrayLength==0){
		nArrayLength=arraySrc.size();
	}
	int w = width;
	int h = height;
	int b = 10;		// border around graph within the image
...
//	CvScalar colorGraph = getGraphColor();	// use a different color each time.
	CvScalar colorGraph = CV_RGB(255,0,255);	// use a different color each time.
	...
	// Draw the horizontal & vertical axis
	int y0 = cvRound(minV*fscale);
	line(imageGraph, cvPoint(b,h-(b-y0)), cvPoint(w-b, h-(b-y0)), BLACK,2);
	line(imageGraph, cvPoint(b,h-(b)), cvPoint(b, h-(b+s)), BLACK,2);
	...
	if (showScale) {
		...
		char text[16];
		//		cvPutText(imageGraph, cv::format("%.1f", maxV), cvPoint(1, b+4), &font, clr);
		putText(imageGraph, cv::format("%.1f", maxV),   Point(1+15, b+4), FONT_HERSHEY_PLAIN, 0.5*2, BLACK, 2);
		// Write the scale of the x axis
		//		sprintf_s(text, sizeof(text)-1, "%d", (nArrayLength-1) );
		//		cvPutText(imageGraph, text, cvPoint(w-b+4-5*strlen(text), (h/2)+10), &font, clr);
		putText(imageGraph, cv::format("%d", (nArrayLength-1+1)),   Point(w-b+4-5*strlen(text), (h/2)+10+10), FONT_HERSHEY_PLAIN, 0.5*2, BLACK, 2);
	}
	...
	if (graphLabel != NULL && strlen(graphLabel) > 0) {
		//cvInitFont(&font,CV_FONT_HERSHEY_PLAIN, 0.5,0.7, 0,1,CV_AA);
//		cvPutText(imageGraph, graphLabel, cvPoint(30, 10), &font, CV_RGB(0,0,0));	// black text
		putText(imageGraph, cv::format("%s", graphLabel),   Point(30,10), FONT_HERSHEY_PLAIN, 0.5, BLACK, 1);
	}
